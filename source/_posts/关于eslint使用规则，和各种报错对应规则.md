...
```
WARNING Compiled with 1 warnings11:26:30http://eslint.org/docs/rules/quotes 
Strings must use singlequote
```
警告的意思就是**字符串必须用单引号**
如下图：
![](http://upload-images.jianshu.io/upload_images/4340772-eb128a705f102481.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
这时候，你就可以在**.eslintrc.js**配置你的规则rule
...
如下图
![](http://upload-images.jianshu.io/upload_images/4340772-355bb877b7ed0ba9.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)这样就可以和Eslint携手并进了

**配置（我主要用第三种方法,为了方便查看，特记录如下：）**
可以通过以下三种方式配置 ESLint:
* 1. 使用 .eslintrc 文件（支持 JSON 和 YAML 两种语法）；
* 2. 在 package.json 中添加 eslintConfig 配置块；直接在代码文件中定义。
* 3. 以下是**.eslintrc** 文件示例和解释：**为了方便查看，特记录如下,需要用的规则，到时候直接搜索就可以了，简直开心极了：**
```
{  
  "env": {  
    "browser": true,  
...
```
参考文章：
[ESLint配置参数介绍](http://link.zhihu.com/?target=https%3A//segmentfault.com/a/1190000004468428)
[关于eslint使用规则，和各种报错应对书写规则。*](http://link.zhihu.com/?target=http%3A//www.cnblogs.com/nklong/p/7233631.html)
